...
let profilesPromise: Promise<Map<string, UMKMProfileOut>> | null = null;
let cachedProfiles: Map<string, UMKMProfileOut> | null = null;
let cachedProfilesTimestamp = 0;
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 menit

/** Profil UMKM per user_id, dikumpulkan dari endpoint yang boleh diakses lender. */
export async function loadProfilesByUser(): Promise<Map<string, UMKMProfileOut>> {
  const now = Date.now();
  if (cachedProfiles && now - cachedProfilesTimestamp < CACHE_TTL_MS) {
  ...
  profilesPromise = (async () => {
    ...
    cachedProfiles = byUser;
    cachedProfilesTimestamp = Date.now();
...
async function loadLatestScoresGrouped(): Promise<Map<string, ScoreOut>> {
  const results = await Promise.all(
  ...
  for (const list of results) {
    for (const score of list ?? []) {
      ...
      if (!existing) {
        ...
        const timeNew = score.created_at ? new Date(score.created_at).getTime() : 0;
...
export async function loadMerchantPipeline(): Promise<Application[]> {
  const [users, profiles, scoreData] = await Promise.all([
    tryFetch<UserOut[]>("/users/?role=umkm&limit=100"),
    loadProfilesByUser(),
    loadLatestScoresGrouped(),
  ]);

  const scoredUserIds = [...scoreData.keys()];
  const userIds = users
    ? users.map((u) => u.id)
  ...
  return userIds.map((userId) => {
    ...
    const score = scoreData.get(userId) ?? null;
